...
ServoCommands servo_calculate(const ControlOutput& control, 
                            float gyro_z, // yaw角速度
                            float gyro_y, // roll角速度
                            float gyro_x, // 当前俯仰角速度
    ...
    // 1. PID计算俯仰舵面指令
    float pitch_cmd = pitch_pid.update(control.pitch_delta, gyro_x, current_time, angle_of_attack);
    ...
    // 滚转控制: 保持稳定(使用当前角度z)
    float yaw_cmd = (control.yaw_delta * Yaw_Control_params.yaw_angle_gain + gyro_z * Yaw_Control_params.yaw_gyro_gain) * Yaw_Control_params.yaw_kp;
    
    float roll_cmd = (control.roll_delta * Roll_Control_params.roll_angle_gain + gyro_y * Roll_Control_params.roll_gyro_gain) * Roll_Control_params.roll_kp;
    ...
    cmd.servo1 = Guidance_params.servo_neutral + cos45*(-pitch_cmd - yaw_cmd) - roll_cmd;
    cmd.servo2 = Guidance_params.servo_neutral + cos45*(pitch_cmd - yaw_cmd) - roll_cmd;
    cmd.servo3 = Guidance_params.servo_neutral + cos45*(-pitch_cmd + yaw_cmd) - roll_cmd;
    cmd.servo4 = Guidance_params.servo_neutral + cos45*(pitch_cmd + yaw_cmd) - roll_cmd;
    
    // 限制舵面指令范围
    cmd.servo1 = std::clamp(cmd.servo1, Guidance_params.servo_min, Guidance_params.servo_max);
    cmd.servo2 = std::clamp(cmd.servo2, Guidance_params.servo_min, Guidance_params.servo_max);
    cmd.servo3 = std::clamp(cmd.servo3, Guidance_params.servo_min, Guidance_params.servo_max);
    cmd.servo4 = std::clamp(cmd.servo4, Guidance_params.servo_min, Guidance_params.servo_max);

    log::info("cmd.servo1=%.1f cmd.servo2=%.1f cmd.servo3=%.1f cmd.servo4=%.1f", 
        cmd.servo1, cmd.servo2, cmd.servo3, cmd.servo4);
...
ServoCommands servo_roll_yaw_calculate(const ControlOutput& control, //需要达到的控制量delta
    ...
    float roll_cmd = (control.roll_delta * Roll_Control_params.roll_angle_gain + gyro_roll * Roll_Control_params.roll_gyro_gain) * Roll_Control_params.roll_kp + 2.5f;
...
// 视频录制线程
// -----------------------------
void video_record_thread_func(camera::Camera *cam) {
    int file_counter = 0;
    std::string base_path = "/root/videos/output";
    ...
    while (running) {
        ...
        if (file_counter == 0) {
            ...
            filepath = base_path + std::to_string(file_counter) + ".mp4";
        }
        
        // 检查文件是否存在，如果存在则递增计数器
        while (maix::fs::exists(filepath.c_str())) {
            file_counter++;
            if (file_counter == 0) {
                filepath = base_path + ".mp4";
            } else {
        ...
        // 设置输出路径
        recorder.config_path(filepath.c_str());
        
        // 开始录制
        recorder.record_start();
        log::info("Video recording started: %s", filepath.c_str());
        ...
        log::info("Video recording finished: %s", filepath.c_str());
...
void process_control_func(ADC &adc, camera::Camera *cam) {
    ...
    while (running) {
        ...
        float angle_of_attack = vol * 360 / 3.37 - 180;
        // if(angle < -60)
        //     angle = angle + 180;
        // else if(angle > 60)
        //     angle = angle - 180;
        //angle_of_attack = angle_of_attack - guidance_params.init_angle;//攻角，让风标水平为0
        ...
        if (!local_centers.empty()) {
            ...
            // 计算舵面指令
            ServoCommands cmd = servo_calculate(
                control, 
                gyro_yaw, // yaw的角速度
                gyro_roll, // roll的角速度
                gyro_pitch, // 当前俯仰角速度
            ...
            // 计算舵面指令 
            ServoCommands cmd = servo_roll_yaw_calculate(
                control, 
                gyro_yaw, // yaw的速度
                gyro_roll, // roll的速度
            ...
            log::info("time=%dms flight_roll=%.1f gyro_roll=%.1f flight_yaw=%.1f gyro_yaw=%.1f flight_pitch=%.1f gyro_pitch=%.1f acc_pitch=%.1f cmd.servo1=%.1f cmd.servo2=%.1f cmd.servo3=%.1f cmd.servo4=%.1f", 
                data_time1 - current_time, flight_roll, gyro_roll, flight_yaw, gyro_yaw, flight_pitch, gyro_pitch, acc_pitch, cmd.servo1, cmd.servo2, cmd.servo3, cmd.servo4);
...
int _main(int argc, char *argv[])
{
    ...
    fs::mkdir("/root/videos", true);
    // 初始化相机
    camera::Camera cam = camera::Camera(320, 240, image::Format::FMT_YVU420SP, nullptr, 90,3, true, false);
    ...
    cam.exposure(1000);// 设置固定的曝光时间，单位us
    // cam.gain(1024);// 设置固定的增益数值
    ...
    // 启动线程时传递相机对象
    std::thread video_thread(video_record_thread_func, &cam);
    std::thread process_thread1(process_control_func, std::ref(adc), nullptr); 
