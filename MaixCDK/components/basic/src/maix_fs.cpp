...
/* __cplusplus宏，由编译器预定义，表示当前C++标准版本
* C++17 对应的值为 201703L（格式：YYYYMM，2017年3月发布）
* 条件编译 (#if)
* 命名空间别名 (fs_sys)
* std是标准库的根命名空间，filesystem是std下的一个子命名空间
*/
#if __cplusplus < 201703L   //检查 C++ 标准版本是否低于 C++17
#include <experimental/filesystem>
namespace fs_sys = std::experimental::filesystem;   // 使用实验性版本
#else
#include <filesystem>
namespace fs_sys = std::filesystem;   // 使用标准版本
#endif
