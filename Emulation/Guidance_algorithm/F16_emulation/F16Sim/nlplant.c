...
/*  Merging the nlplant.c (lofi) and nlplant_hifi.c to use
    ...
    合并nlplant.c （lofi）和nlplant_hifi.c以使用
    相同的运动方程，导航方程和使用由标志决定的查找表。                  */
    
void atmos(double,double,double*);          /* Used by both */
void accels(double*,double*,double*);       /* Used by both */

#include "lofi_F16_AeroData.c"              /* LOFI Look-up header file低保真查找头文件*/
#include "hifi_F16_AeroData.c"              /* HIFI Look-up header file高保真查找头文件*/

void nlplant(double*,double*);

/*########################################*/
/*### Added for mex function in matlab增加了matlab中的mex函数 ###*/
/*### 因为这个代码使用c编写，matlab不能直接识别 ###*/
/*########################################*/
