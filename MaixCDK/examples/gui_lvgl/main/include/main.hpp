...
struct GuidanceParams {
    ...
    float servo_neutral; //舵机中立位置
    float servo_min; //舵机最小位置限幅
    float servo_max; //舵机最大位置限幅
...
class PIDController {
    public:
        ...
        float update(float setpoint, float measurement, uint64_t current_time, float angle) {
            // 计算时间步长（秒）
...
class AsyncLogger {
        ...
        void log_thread_func() {
            while(running) {
                ...
                if((!temp_buffer.empty() || do_flush) && log_file) {
                    ...
                    if(!log_file->flush()) {
                        maix::log::error("Failed to flush log data");
                    }
                }
            }
        }
    ...
    public:
        ...
        : buffer_size(buf_size), last_flush_time(std::chrono::steady_clock::now()) {
            ...
            if(log_file) {
                // 写入CSV头部
                std::string header = "timestamp,flight_roll,gyro_roll,flight_yaw,gyro_yaw,flight_pitch,gyro_pitch,acc_pitch,cmd_servo1,cmd_servo2,cmd_servo3,cmd_servo4\n";
