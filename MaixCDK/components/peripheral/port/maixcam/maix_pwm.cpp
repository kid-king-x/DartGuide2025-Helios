...
namespace maix::peripheral::pwm
{
        ...
        : _duty_fd(-1), _enable_fd(-1)
    {
        ...
        snprintf(period_path, sizeof(period_path), PERIOD_PATH, _chip_id, _id_offset);
        int period_fd = ::open(period_path, O_WRONLY);
        if (period_fd >= 0) {
            char period_buf[32];
            snprintf(period_buf, sizeof(period_buf), "%d", _period_ns);
            if (::write(period_fd, period_buf, strlen(period_buf)) != (int)strlen(period_buf)) {
                ::close(period_fd);
                ::close(_duty_fd);
                ::close(_enable_fd);
                throw err::Exception(err::Err::ERR_IO, "set period failed");
            }
            ::close(period_fd);
        } else {
            ::close(_duty_fd);
            ::close(_enable_fd);
            throw err::Exception(err::Err::ERR_IO, "open period failed");
        }

        // 设置初始占空比
        char duty_buf[32];
        snprintf(duty_buf, sizeof(duty_buf), "%d", _duty_val);
        if (::write(_duty_fd, duty_buf, strlen(duty_buf)) != (int)strlen(duty_buf)) {
            ::close(_duty_fd);
            ::close(_enable_fd);
        ...
        // 设置使能状态
        char enable_buf[32];
        snprintf(enable_buf, sizeof(enable_buf), "%d", _enable ? 1 : 0);
        if (::write(_enable_fd, enable_buf, strlen(enable_buf)) != (int)strlen(enable_buf)) {
            ::close(_duty_fd);
            ::close(_enable_fd);
