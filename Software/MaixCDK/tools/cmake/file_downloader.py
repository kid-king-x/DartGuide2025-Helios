...
def download_extract_files(items):
    ...
    for item in items:
        # 跳过 ax620e_msp 的资源下载（手动补丁）
        if "ax620e_msp" in item.get("filename", ""):
            print(f"-- Skip downloading {item['filename']} (manually patched in file_downloader.py)")
            continue  # 跳过当前文件的下载和解压流程
